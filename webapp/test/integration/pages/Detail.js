						...
						success: function(aEntitySet) {
							...
							return this.waitFor({
								...
								check: function(oList) {
									...
									var sObjectID = oList.getBindingContext().getProperty("SALESORDERNUMBER");

									var iLength = aEntitySet.filter(function(oLineItem) {
										return oLineItem.SALESORDERNUMBER === sObjectID;
									}).length;

									return oList.getItems().length === iLength;
								},
								success: function() {
									QUnit.ok(true, "The list has the correct number of items");
								},
				...
				theShareTileButtonShouldContainTheRememberedObjectName: function() {
					return this.waitFor({
						...
						matchers: function(oButton) {
							var sObjectName = this.getContext().currentListItem.getBindingContext().getProperty("SALESORDERNUMBER");
